...
export function buildRepoNamespace(repoFullName: string) {
  ...
  export async function saveRepoChunks(namespace: string, chunks: CodeChunk[]) {
    const index = getPineconeIndex();
    const embeddable = chunks.filter((chunk) => chunk.text.trim().length > 0);

    for (let start = 0; start < embeddable.length; start += UPSERT_BATCH_SIZE) {
      const batch = embeddable.slice(start, start + UPSERT_BATCH_SIZE);
