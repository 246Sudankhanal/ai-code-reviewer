...
function requireEnv(name: "PINECONE_API_KEY" | "PINECONE_INDEX") {
  ...
  if (!value) {
    throw new NonRetriableError(
      `Missing ${name} in .env. Add PINECONE_API_KEY from Pinecone → API keys, and PINECONE_INDEX as the index name you created (e.g. ai-code-reviewer). Restart npm run dev after saving.`
    );
  }
